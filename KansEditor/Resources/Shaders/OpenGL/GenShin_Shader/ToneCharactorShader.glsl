...
void main()
{
	...
	vec4 BaseColor = texture(material.U_DiffuseTexture,vec2(V_TexCroods.x,V_TexCroods.y));
	
    vec4 LightMapColor = texture(material.U_ToneLightMap,vec2(V_TexCroods.x,1.0-V_TexCroods.y));
	//Prep
	//lambert(LoN)
	...
	if(_UseRamp)
	{
		...
		//tights
		vec3 rampShadowColor1 = pow(texture(material.U_ToneRampMap,vec2(rampU,0.05)).rgb,vec3(1.0/2.2));
		// hard/emission/specular/silk
		vec3 rampShadowColor2 = pow(texture(material.U_ToneRampMap,vec2(rampU,0.15)).rgb,vec3(1.0/2.2));
		//metal
		vec3 rampShadowColor3 = pow(texture(material.U_ToneRampMap,vec2(rampU,0.25)).rgb,vec3(1.0/2.2));
		//skin
		vec3 rampShadowColor4 = pow(texture(material.U_ToneRampMap,vec2(rampU,0.35)).rgb,vec3(1.0/2.2));
		//soft/common
		vec3 rampShadowColor5 = pow(texture(material.U_ToneRampMap,vec2(rampU,0.45)).rgb,vec3(1.0/2.2));
		
		vec3 rampShadowColorCool1 = texture(material.U_ToneRampMap,vec2(rampU,0.55)).rgb;
		vec3 rampShadowColorCool2 = texture(material.U_ToneRampMap,vec2(rampU,0.65)).rgb;
		vec3 rampShadowColorCool3 = texture(material.U_ToneRampMap,vec2(rampU,0.75)).rgb;
		vec3 rampShadowColorCool4 = texture(material.U_ToneRampMap,vec2(rampU,0.85)).rgb;
		vec3 rampShadowColorCool5 = texture(material.U_ToneRampMap,vec2(rampU,0.95)).rgb;
